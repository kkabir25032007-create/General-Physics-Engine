...
class vector:
    ...
    def __init__(self, i=0, j=0, k=0):      # component will take value 0 if not specified. 
        ...
        if self.magnitude != 0:    
            self.angle_x = math.acos(max(-1, min(1, self.i/self.magnitude)))
            
            self.angle_y = math.acos(max(-1, min(1, self.j/self.magnitude)))
            
            self.angle_z = math.acos(max(-1, min(1, self.k/self.magnitude)))
        else:
            self.angle_x = 0
            self.angle_y = 0
            self.angle_z = 0
